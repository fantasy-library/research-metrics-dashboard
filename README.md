...
- Optional backups: `SCIVAL_API_KEY_1` and `SCIVAL_API_KEY_2`. On HTTP 401 the app retries these keys **without** `insttoken` (useful on Railway when backups do not need an institutional token). Applies to SciVal metrics and Publications Analysis (Scopus search / ASJC).
