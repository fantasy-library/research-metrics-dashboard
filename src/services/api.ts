...
export class APIService {
  ...
  private async getDirectAuthorMetrics(authorId: string, customApiKey?: string, yearRange: string = '5yrs', availableMetrics?: any[], includedDocs: string = 'AllPublicationTypes'): Promise<AuthorMetrics> {
    try {
      ...
      const hIndexData = await this.fetchDirectMetric(authorId, 'HIndices', false, customApiKey, yearRange, 'false', includedDocs);
      ...
      if (enabledMetricIds.includes('citations')) {
        await new Promise(resolve => setTimeout(resolve, 200));
        citationYearData = await this.fetchDirectMetric(authorId, 'CitationCount', true, customApiKey, yearRange, 'false', includedDocs);
        
        await new Promise(resolve => setTimeout(resolve, 200));
        citationTotalData = await this.fetchDirectMetric(authorId, 'CitationCount', false, customApiKey, yearRange, 'false', includedDocs);
      }

      if (enabledMetricIds.includes('citationsIncl')) {
        await new Promise(resolve => setTimeout(resolve, 200));
        citationInclYearData = await this.fetchDirectMetric(authorId, 'CitationCount', true, customApiKey, yearRange, 'true', includedDocs);
        
        await new Promise(resolve => setTimeout(resolve, 200));
        citationInclTotalData = await this.fetchDirectMetric(authorId, 'CitationCount', false, customApiKey, yearRange, 'true', includedDocs);
      }
      ...
      if (citationYearData && citationTotalData) {
        metrics.citations = this.processCitations(citationYearData, citationTotalData);
      } else {
        metrics.citations = { byYear: {}, total: 'N/A' };
      }

      if (citationInclYearData && citationInclTotalData) {
        metrics.citationsIncl = this.processCitations(citationInclYearData, citationInclTotalData);
      } else {
  ...
  async getAuthorMetrics(authorId: string, customApiKey?: string, yearRange: string = '5yrs', availableMetrics?: any[], includedDocs: string = 'AllPublicationTypes'): Promise<AuthorMetrics> {
    try {
      if (USE_DIRECT_API) {
        return await this.getDirectAuthorMetrics(authorId, customApiKey, yearRange, availableMetrics, includedDocs);
      } else {
  ...
  async processMultipleAuthors(authorIds: string[], customApiKey?: string, yearRange: string = '5yrs', availableMetrics?: any[], includedDocs: string = 'AllPublicationTypes'): Promise<Array<{id: string, data: AuthorMetrics}>> {
    try {
      if (USE_DIRECT_API) {
        ...
        for (const authorId of authorIds) {
          try {
            ...
            const data = await this.getDirectAuthorMetrics(authorId.trim(), customApiKey, yearRange, availableMetrics, includedDocs);
            ...
            results.push({
              ...
              data: {
                ...
                metrics: {
                  ...
                  citations: { byYear: {}, total: 'N/A' },
                  citationsIncl: { byYear: {}, total: 'N/A' },
